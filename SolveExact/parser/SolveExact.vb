'Generated by the GOLD Parser Builder

Option Explicit On
Option Strict Off

Imports System.IO
Imports System.Windows.Forms


Module MyParser
    Private Parser As New GOLD.Parser

    Private Enum SymbolIndex
        [Eof] = 0                                 ' (EOF)
        [Error] = 1                               ' (Error)
        [Whitespace] = 2                          ' Whitespace
        [Minus] = 3                               ' '-'
        [Minusminus] = 4                          ' '--'
        [Lparen] = 5                              ' '('
        [Rparen] = 6                              ' ')'
        [Times] = 7                               ' '*'
        [Comma] = 8                               ' ','
        [Div] = 9                                 ' '/'
        [Semi] = 10                               ' ';'
        [Lbracket] = 11                           ' '['
        [Rbracket] = 12                           ' ']'
        [Caret] = 13                              ' '^'
        [Lbrace] = 14                             ' '{'
        [Rbrace] = 15                             ' '}'
        [Plus] = 16                               ' '+'
        [Plusplus] = 17                           ' '++'
        [Eq] = 18                                 ' '='
        [Arccos] = 19                             ' arccos
        [Arcsin] = 20                             ' arcsin
        [Arctan] = 21                             ' arctan
        [Cos] = 22                                ' cos
        [Cot] = 23                                ' cot
        [Csc] = 24                                ' csc
        [Definir] = 25                            ' definir
        [E] = 26                                  ' e
        [Exp] = 27                                ' exp
        [Float] = 28                              ' Float
        [Graph] = 29                              ' graph
        [Id] = 30                                 ' ID
        [Integer] = 31                            ' Integer
        [Ln] = 32                                 ' ln
        [Log] = 33                                ' log
        [Log2] = 34                               ' 'log2'
        [Pi] = 35                                 ' pi
        [Sec] = 36                                ' sec
        [Sin] = 37                                ' sin
        [Sqrt] = 38                               ' sqrt
        [Tan] = 39                                ' tan
        [Var] = 40                                ' var
        [Assign] = 41                             ' <Assign>
        [Call] = 42                               ' <Call>
        [Declaration] = 43                        ' <Declaration>
        [Expression] = 44                         ' <Expression>
        [Funcassign] = 45                         ' <Func Assign>
        [Funcdef] = 46                            ' <Func Def>
        [Listexp] = 47                            ' <List Exp>
        [Listid] = 48                             ' <List ID>
        [Multexp] = 49                            ' <Mult Exp>
        [Postunaryexp] = 50                       ' <PostUnary Exp>
        [Powexp] = 51                             ' <Pow Exp>
        [Start] = 52                              ' <Start>
        [Unaryexp] = 53                           ' <Unary Exp>
        [Value] = 54                              ' <Value>
    End Enum

    Private Enum ProductionIndex
        [Start] = 0                               ' <Start> ::= <Declaration>
        [Start2] = 1                              ' <Start> ::= <Func Def>
        [Start3] = 2                              ' <Start> ::= <Expression>
        [Start_Graph_Lparen_Rparen_Semi] = 3      ' <Start> ::= graph '(' <Expression> ')' ';'
        [Declaration_Var_Id_Semi] = 4             ' <Declaration> ::= var ID ';'
        [Declaration_Var] = 5                     ' <Declaration> ::= var <Assign>
        [Declaration] = 6                         ' <Declaration> ::= <Assign>
        [Assign_Id_Eq_Semi] = 7                   ' <Assign> ::= ID '=' <Expression> ';'
        [Funcdef_Definir_Id_Lparen_Rparen_Semi] = 8 ' <Func Def> ::= definir ID '(' <List ID> ')' ';'
        [Funcdef_Definir] = 9                     ' <Func Def> ::= definir <Func Assign>
        [Funcdef_Semi] = 10                       ' <Func Def> ::= <Expression> ';'
        [Funcassign_Id_Lparen_Rparen_Eq_Semi] = 11 ' <Func Assign> ::= ID '(' <List ID> ')' '=' <Expression> ';'
        [Listid_Comma_Id] = 12                    ' <List ID> ::= <List ID> ',' ID
        [Listid_Id] = 13                          ' <List ID> ::= ID
        [Expression_Plus] = 14                    ' <Expression> ::= <Expression> '+' <Mult Exp>
        [Expression_Minus] = 15                   ' <Expression> ::= <Expression> '-' <Mult Exp>
        [Expression] = 16                         ' <Expression> ::= <Mult Exp>
        [Multexp_Times] = 17                      ' <Mult Exp> ::= <Mult Exp> '*' <Pow Exp>
        [Multexp_Div] = 18                        ' <Mult Exp> ::= <Mult Exp> '/' <Pow Exp>
        [Multexp] = 19                            ' <Mult Exp> ::= <Pow Exp>
        [Powexp_Caret] = 20                       ' <Pow Exp> ::= <Pow Exp> '^' <Unary Exp>
        [Powexp] = 21                             ' <Pow Exp> ::= <Unary Exp>
        [Unaryexp_Minus] = 22                     ' <Unary Exp> ::= '-' <Value>
        [Unaryexp_Plusplus_Id] = 23               ' <Unary Exp> ::= '++' ID
        [Unaryexp_Minusminus_Id] = 24             ' <Unary Exp> ::= '--' ID
        [Unaryexp] = 25                           ' <Unary Exp> ::= <PostUnary Exp>
        [Unaryexp2] = 26                          ' <Unary Exp> ::= <Value>
        [Postunaryexp_Id_Plusplus] = 27           ' <PostUnary Exp> ::= ID '++'
        [Postunaryexp_Id_Minusminus] = 28         ' <PostUnary Exp> ::= ID '--'
        [Postunaryexp] = 29                       ' <PostUnary Exp> ::= <Call>
        [Call_Id_Lparen_Rparen] = 30              ' <Call> ::= ID '(' <List Exp> ')'
        [Value_E] = 31                            ' <Value> ::= e
        [Value_Pi] = 32                           ' <Value> ::= pi
        [Value_Id] = 33                           ' <Value> ::= ID
        [Value_Float] = 34                        ' <Value> ::= Float
        [Value_Integer] = 35                      ' <Value> ::= Integer
        [Value_Sin_Lparen_Rparen] = 36            ' <Value> ::= sin '(' <Expression> ')'
        [Value_Cos_Lparen_Rparen] = 37            ' <Value> ::= cos '(' <Expression> ')'
        [Value_Tan_Lparen_Rparen] = 38            ' <Value> ::= tan '(' <Expression> ')'
        [Value_Csc_Lparen_Rparen] = 39            ' <Value> ::= csc '(' <Expression> ')'
        [Value_Sec_Lparen_Rparen] = 40            ' <Value> ::= sec '(' <Expression> ')'
        [Value_Cot_Lparen_Rparen] = 41            ' <Value> ::= cot '(' <Expression> ')'
        [Value_Arcsin_Lparen_Rparen] = 42         ' <Value> ::= arcsin '(' <Expression> ')'
        [Value_Arccos_Lparen_Rparen] = 43         ' <Value> ::= arccos '(' <Expression> ')'
        [Value_Arctan_Lparen_Rparen] = 44         ' <Value> ::= arctan '(' <Expression> ')'
        [Value_Ln_Lparen_Rparen] = 45             ' <Value> ::= ln '(' <Expression> ')'
        [Value_Log2_Lparen_Rparen] = 46           ' <Value> ::= 'log2' '(' <Expression> ')'
        [Value_Log_Lparen_Rparen] = 47            ' <Value> ::= log '(' <Expression> ')'
        [Value_Exp_Lparen_Rparen] = 48            ' <Value> ::= exp '(' <Expression> ')'
        [Value_Sqrt_Lparen_Rparen] = 49           ' <Value> ::= sqrt '(' <Expression> ')'
        [Value_Lbracket_Rbracket] = 50            ' <Value> ::= '[' <Expression> ']'
        [Value_Lbrace_Rbrace] = 51                ' <Value> ::= '{' <Expression> '}'
        [Value_Lparen_Rparen] = 52                ' <Value> ::= '(' <Expression> ')'
        [Listexp_Comma] = 53                      ' <List Exp> ::= <List Exp> ',' <Expression>
        [Listexp] = 54                            ' <List Exp> ::= <Expression>
    End Enum

    Public Program As Object     'You might derive a specific object

    Public Sub Setup()
        'This procedure can be called to load the parse tables. The class can
        'read tables using a BinaryReader.
        
        Parser.LoadTables(Path.Combine(Application.StartupPath, "solve_exact.egt"))
    End Sub
    
    Public Function Parse(ByVal Reader As TextReader) As Boolean
        'This procedure starts the GOLD Parser Engine and handles each of the
        'messages it returns. Each time a reduction is made, you can create new
        'custom object and reassign the .CurrentReduction property. Otherwise, 
        'the system will use the Reduction object that was returned.
        '
        'The resulting tree will be a pure representation of the language 
        'and will be ready to implement.

        Dim Response As GOLD.ParseMessage
        Dim Done as Boolean                  'Controls when we leave the loop
        Dim Accepted As Boolean = False      'Was the parse successful?

        Accepted = False    'Unless the program is accepted by the parser

        Parser.Open(Reader)
        Parser.TrimReductions = False  'Please read about this feature before enabling  

        Done = False
        Do Until Done
            Response = Parser.Parse()

            Select Case Response              
                Case GOLD.ParseMessage.LexicalError
                    'Cannot recognize token
                    ErrorManager.Add("Error léxico: token  " & Parser.CurrentToken.Data & " Col: " & Parser.CurrentPosition.Column)
                    Parser.DiscardCurrentToken()
                    Done = False

                Case GOLD.ParseMessage.SyntaxError
                    'Expecting a different token
                    ErrorManager.Add("Error de sintáxis: token " & Parser.CurrentToken.Data & " se esperaba: " + Parser.ExpectedSymbols.ToString & " Col:" & Parser.CurrentPosition.Column)
                    Done = True

                Case GOLD.ParseMessage.Reduction
                    'Create a customized object to store the reduction
                    Parser.CurrentReduction = CreateNewObject(Parser.CurrentReduction)

                Case GOLD.ParseMessage.Accept
                    'Accepted!
                    'Program = Parser.CurrentReduction  'The root node!                 
                    Done = True
                    Accepted = True

                Case GOLD.ParseMessage.TokenRead
                    'You don't have to do anything here.

                Case GOLD.ParseMessage.InternalError
                    'INTERNAL ERROR! Something is horribly wrong.
                    Done = True

                Case GOLD.ParseMessage.NotLoadedError
                    'This error occurs if the CGT was not loaded.                   
                    Done = True

                Case GOLD.ParseMessage.GroupError 
                    'COMMENT ERROR! Unexpected end of file
                    Done = True
            End Select
        Loop

        Return Accepted
    End Function

    Private Function CreateNewObject(Reduction as GOLD.Reduction) As Object
        Dim Result As Object = Nothing

        With Reduction
            Select Case .Parent.TableIndex                        
                Case ProductionIndex.Start                 
                    ' <Start> ::= <Declaration> 
                    Command.AddCommand(.Item(0).Data)

                Case ProductionIndex.Start2                 
                    ' <Start> ::= <Func Def>
                    Command.AddCommand(.Item(0).Data)

                Case ProductionIndex.Start3                 
                    ' <Start> ::= <Expression> 
                    Command.AddCommand(.Item(0).Data)

                Case ProductionIndex.Start_Graph_Lparen_Rparen_Semi                 
                    ' <Start> ::= graph '(' <Expression> ')' ';'
                    Command.AddCommand(New GraphExp(.Item(2).Data))


                Case ProductionIndex.Declaration_Var_Id_Semi                 
                    ' <Declaration> ::= var ID ';'
                    Return New DecVar(CType(.Item(0).Data, String))

                Case ProductionIndex.Declaration_Var                 
                    ' <Declaration> ::= var <Assign> 
                    Return New DecVar(CType(.Item(1).Data, AssignVar))

                Case ProductionIndex.Declaration                 
                    ' <Declaration> ::= <Assign> 
                    Return .Item(0).Data

                Case ProductionIndex.Assign_Id_Eq_Semi                 
                    ' <Assign> ::= ID '=' <Expression> ';' 
                    Return New AssignVar(.Item(0).Data, .Item(2).Data)

                Case ProductionIndex.Funcdef_Definir_Id_Lparen_Rparen_Semi                 
                    ' <Func Def> ::= definir ID '(' <List ID> ')' ';' 
                    Return New DecFunc(.Item(1).Data, .Item(3).Data)

                Case ProductionIndex.Funcdef_Definir                 
                    ' <Func Def> ::= definir <Func Assign> 
                    Return .Item(1).Data

                Case ProductionIndex.Funcdef_Semi                 
                    ' <Func Def> ::= <Expression> ';' 
                    Return .Item(0).Data

                Case ProductionIndex.Funcassign_Id_Lparen_Rparen_Eq_Semi                 
                    ' <Func Assign> ::= ID '(' <List ID> ')' '=' <Expression> ';' 
                    Return New DecFunc(.Item(0).Data, .Item(2).Data, .Item(5).Data)

                Case ProductionIndex.Listid_Comma_Id                 
                    ' <List ID> ::= <List ID> ',' ID
                    Dim list_id = CType(.Item(0).Data, Stack(Of String))
                    list_id.Push(.Item(2).Data)
                    Return list_id

                Case ProductionIndex.Listid_Id                 
                    ' <List ID> ::= ID
                    Dim list_id = New Stack(Of String)
                    list_id.Push(.Item(0).Data)
                    Return list_id

                Case ProductionIndex.Expression_Plus                 
                    ' <Expression> ::= <Expression> '+' <Mult Exp> 
                    Return New AritOp(.Item(0).Data, .Item(2).Data, CType(.Item(1).Data, String))

                Case ProductionIndex.Expression_Minus                 
                    ' <Expression> ::= <Expression> '-' <Mult Exp> 
                    Return New AritOp(.Item(0).Data, .Item(2).Data, CType(.Item(1).Data, String))

                Case ProductionIndex.Expression                 
                    ' <Expression> ::= <Mult Exp> 
                    Return .Item(0).Data

                Case ProductionIndex.Multexp_Times                 
                    ' <Mult Exp> ::= <Mult Exp> '*' <Pow Exp> 
                    Return New AritOp(.Item(0).Data, .Item(2).Data, CType(.Item(1).Data, String))

                Case ProductionIndex.Multexp_Div                 
                    ' <Mult Exp> ::= <Mult Exp> '/' <Pow Exp> 
                    Return New AritOp(.Item(0).Data, .Item(2).Data, CType(.Item(1).Data, String))

                Case ProductionIndex.Multexp                 
                    ' <Mult Exp> ::= <Pow Exp> 
                    Return .Item(0).Data()

                Case ProductionIndex.Powexp_Caret                 
                    ' <Pow Exp> ::= <Pow Exp> '^' <Unary Exp>
                    Return New AritOp(.Item(0).Data, .Item(2).Data, CType(.Item(1).Data, String))

                Case ProductionIndex.Powexp                 
                    ' <Pow Exp> ::= <Unary Exp>
                    Return .Item(0).Data

                Case ProductionIndex.Unaryexp_Minus                 
                    ' <Unary Exp> ::= '-' <Value> 
                    Return New AritOp(.Item(1).Data, CType(.Item(0).Data, String))

                Case ProductionIndex.Unaryexp_Plusplus_Id                 
                    ' <Unary Exp> ::= '++' ID 
                    Return New AritOp(New GetValue(.Item(1).Data), CType(.Item(0).Data, String))

                Case ProductionIndex.Unaryexp_Minusminus_Id                 
                    ' <Unary Exp> ::= '--' ID 
                    Return New AritOp(New GetValue(.Item(1).Data), CType(.Item(0).Data, String))

                Case ProductionIndex.Unaryexp                 
                    ' <Unary Exp> ::= <PostUnary Exp> 
                    Return .Item(0).Data

                Case ProductionIndex.Unaryexp2                 
                    ' <Unary Exp> ::= <Value> 
                    Return .Item(0).Data

                Case ProductionIndex.Postunaryexp_Id_Plusplus                 
                    ' <PostUnary Exp> ::= ID '++'
                    Return New AritOp(New GetValue(.Item(0).Data), CType(.Item(1).Data, String), True)

                Case ProductionIndex.Postunaryexp_Id_Minusminus                 
                    ' <PostUnary Exp> ::= ID '--' 
                    Return New AritOp(New GetValue(.Item(0).Data), CType(.Item(1).Data, String), True)

                Case ProductionIndex.Postunaryexp                 
                    ' <PostUnary Exp> ::= <Call> 
                    Return .Item(0).Data

                Case ProductionIndex.Call_Id_Lparen_Rparen                 
                    ' <Call> ::= ID '(' <List Exp> ')' 
                    Return New EvaluateFunc(.Item(0).Data, .Item(2).Data)

                Case ProductionIndex.Value_E                 
                    ' <Value> ::= e 
                    Return New Num(Math.E)

                Case ProductionIndex.Value_Pi                 
                    ' <Value> ::= pi 
                    Return New Num(Math.PI)

                Case ProductionIndex.Value_Id                 
                    ' <Value> ::= ID 
                    Return New GetValue(.Item(0).Data)

                Case ProductionIndex.Value_Float                 
                    ' <Value> ::= Float 
                    Return New Num(.Item(0).Data)

                Case ProductionIndex.Value_Integer                 
                    ' <Value> ::= Integer 
                    Return New Num(.Item(0).Data)

                Case ProductionIndex.Value_Sin_Lparen_Rparen
                    ' <Value> ::= sin '(' <Expression> ')' 
                    Return New TrigFunc(.Item(2).Data, .Item(0).Data)

                Case ProductionIndex.Value_Cos_Lparen_Rparen
                    ' <Value> ::= cos '(' <Expression> ')' 
                    Return New TrigFunc(.Item(2).Data, .Item(0).Data)

                Case ProductionIndex.Value_Tan_Lparen_Rparen
                    ' <Value> ::= tan '(' <Expression> ')' 
                    Return New TrigFunc(.Item(2).Data, .Item(0).Data)

                Case ProductionIndex.Value_Csc_Lparen_Rparen
                    ' <Value> ::= csc '(' <Expression> ')' 
                    Return New TrigFunc(.Item(2).Data, .Item(0).Data)

                Case ProductionIndex.Value_Sec_Lparen_Rparen
                    ' <Value> ::= sec '(' <Expression> ')'
                    Return New TrigFunc(.Item(2).Data, .Item(0).Data)

                Case ProductionIndex.Value_Cot_Lparen_Rparen
                    ' <Value> ::= cot '(' <Expression> ')' 
                    Return New TrigFunc(.Item(2).Data, .Item(0).Data)

                Case ProductionIndex.Value_Arcsin_Lparen_Rparen
                    ' <Value> ::= arcsin '(' <Expression> ')' 
                    Return New TrigFunc(.Item(2).Data, .Item(0).Data)

                Case ProductionIndex.Value_Arccos_Lparen_Rparen
                    ' <Value> ::= arccos '(' <Expression> ')' 
                    Return New TrigFunc(.Item(2).Data, .Item(0).Data)

                Case ProductionIndex.Value_Arctan_Lparen_Rparen
                    ' <Value> ::= arctan '(' <Expression> ')' 
                    Return New TrigFunc(.Item(2).Data, .Item(0).Data)

                Case ProductionIndex.Value_Ln_Lparen_Rparen
                    ' <Value> ::= ln '(' <Expression> ')' 
                    Return New Log(.Item(2).Data, New Num(Math.E))

                Case ProductionIndex.Value_Log2_Lparen_Rparen
                    ' <Value> ::= 'log2' '(' <Expression> ')' 
                    Return New Log(.Item(2).Data, New Num(2))

                Case ProductionIndex.Value_Log_Lparen_Rparen
                    ' <Value> ::= log '(' <Expression> ')' 
                    Return New Log(.Item(2).Data, New Num(10))

                Case ProductionIndex.Value_Exp_Lparen_Rparen
                    ' <Value> ::= exp '(' <Expression> ')'
                    Return New AritOp(New Num(Math.E), .Item(2).Data, "^")

                Case ProductionIndex.Value_Sqrt_Lparen_Rparen
                    ' <Value> ::= sqrt '(' <Expression> ')' 
                    Return New AritOp(.Item(2).Data, New Num(0.5), "^")

                Case ProductionIndex.Value_Lbracket_Rbracket                 
                    ' <Value> ::= '[' <Expression> ']'
                    Return .Item(1).Data

                Case ProductionIndex.Value_Lbrace_Rbrace                 
                    ' <Value> ::= '{' <Expression> '}' 
                    Return .Item(1).Data

                Case ProductionIndex.Value_Lparen_Rparen                 
                    ' <Value> ::= '(' <Expression> ')'
                    Return .Item(1).Data

                Case ProductionIndex.Listexp_Comma                 
                    ' <List Exp> ::= <List Exp> ',' <Expression> 
                    Dim stack_exp = CType(.Item(0).Data, Stack(Of Node))
                    stack_exp.Push(.Item(2).Data)
                    Return stack_exp

                Case ProductionIndex.Listexp                 
                    ' <List Exp> ::= <Expression> 
                    Dim stack_exp As New Stack(Of Node)
                    stack_exp.Push(.Item(0).Data)
                    Return stack_exp

            End Select
        End With     

        Return Result
    End Function
End Module
